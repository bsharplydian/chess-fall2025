...
public class WebSocketFacade extends Endpoint {
    ...
    public void makeMove(String authToken, int gameID, ChessMove move) throws HttpResponseException {
        try {
    ...
    public void leave(String authToken, int gameID) throws HttpResponseException {
        try {
            UserGameCommand command = new LeaveCommand(UserGameCommand.CommandType.LEAVE, authToken, gameID);
            this.session.getBasicRemote().sendText(new Gson().toJson(command));
        } catch (IOException e) {
            throw new HttpResponseException(e.getMessage());
        }
    }
    public void resign(String authToken, int gameID) throws HttpResponseException {
        try {
            UserGameCommand command = new ResignCommand(UserGameCommand.CommandType.RESIGN, authToken, gameID);
            this.session.getBasicRemote().sendText(new Gson().toJson(command));
        } catch (IOException e) {
            throw new HttpResponseException(e.getMessage());
        }
